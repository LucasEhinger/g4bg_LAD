...
g4rcScintDetector::g4rcScintDetector( G4String name, G4int detnum ) : G4VSensitiveDetector(name){
    char colname[255];
...
g4rcScintDetector::~g4rcScintDetector(){
}

void g4rcScintDetector::Initialize(G4HCofThisEvent *){
    fHitColl = new g4rcScintDetectorHitsCollection( SensitiveDetectorName, collectionName[0] );
...
G4bool g4rcScintDetector::ProcessHits( G4Step *step, G4TouchableHistory *){
...
//    printf("Hit in %s by %s with edep %f MeV\n", hist->GetVolume()->GetName().data(), track->GetParticleDefinition()->GetParticleName().data(), edep/MeV  );
    ...
    g4rcScintDetectorHit *thissum = NULL;

    if( !fSumMap.count(copyID) ){
	if( edep > 0.0 ){
	    thissum = new g4rcScintDetectorHit(fDetNo, copyID);
...
void g4rcScintDetector::EndOfEvent(G4HCofThisEvent*HCE) {
